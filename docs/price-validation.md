...
```sh
telliot report -a <account> -qt validated-feed-usd-spot -ncr --fetch-flex -gm 40 --use-estimate-fee -pvc majority -pvm percentual_change
```
...
The acceptable string values for `-pvm` are one of `percentual_change`, `percentual_difference` and `absolute_difference`. It will also be used as a query parameter for the price service.

Percentual change algorithm - $\frac{|PriceSubmitted - PriceService|}{PriceService} * 100$

Percentual difference algorithm - $\frac{|PriceSubmitted - PriceService|}{\frac{PriceSubmitted + PriceService}{2}} * 100 $

Absolute difference algorithm - $|PriceSubmitted - PriceService|$

The default price validation method is the `percentual_change` and the default consensus method is the `majority`. The `PRICE_DIFF_ABSOLUTE_TOLERANCE` environment variable tells the tolerance, so that once we have the result of a validation method the validator evaluates if the price is valid or not. For example, given that Telliot is going to submit a PLS/USD price as 0.00014, but the Coingecko price service retrieved the price as 0.00013122, the percentual change of these values is:

$\frac{|0.00014 - 0.00013122|}{0.00013122} * 100 = 6.691053193110791$

If the percentual result of 6.691053193110791 is within the `PRICE_DIFF_ABSOLUTE_TOLERANCE` tolerance the price is evaluated as valid. For instance, if the setup was `PRICE_DIFF_ABSOLUTE_TOLERANCE="7"` the price would be valid, but if it was less than 6.69 it would not be valid.
...
Request URL: http://127.0.0.1:3333/validate-price?price=0.00013189637369191059&tolerance=0.104&validation-method=percentual_change&consensus=majority
Validation method: percentual_change
Consensus: majority
Tolerance: 0.104
